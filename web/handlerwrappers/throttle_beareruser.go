...
// ThrottleBearerUser throttles by uid extracted from the user-bearer-session in ctx.
type ThrottleBearerUser[UID comparable] struct {
	AppProvider   framework.AppProviderFunc
	BucketGroupID string
}

func (m *ThrottleBearerUser[UID]) Wrap(inner http.Handler) http.Handler {
